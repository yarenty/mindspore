...
class MDInvoke(object):
    ...
    def __str__(self) -> str:
        return f"MDInvoke(task_name={self.task_name}, input_binding={self.input_binding}, "\
...
class MDTask(object):
    def __init__(self, path: str):
        """
        Args:
            path (str): Path to a mind-docker task file.
        """
        path = os.path.abspath(path)
        metadata, err = MLObject.create_object_from_file(path)
        if err:
    ...
    def __str__(self) -> str:
        return f"MDTask(inputs={self.inputs}, outputs={self.outputs})"


class MD(object):
    """Provides access to limited number of mind-docker parameters."""
    def __init__(self, path: str):
        """
        Args:
            path (str): Path to a mind-docker root directory.
        """
        path = os.path.abspath(path)
        metadata, err = MLObject.create_object_from_file(os.path.join(path, 'md.yaml'))
        if err:
            raise RuntimeError(err)
        ...
        self.task: Optional[MDTask] = None
        self.invoke: Optional[MDInvoke] = None
        self.platform: Any = None

    @property
    def qualified_name(self) -> str:
        """Return a fully qualified name of this mind-docker: '{name}-{version}'."""
    ...
    def __str__(self) -> str:
        return f"MD(root={self.root}, name={self.name}, version={self.version}, task={self.task}, "\
