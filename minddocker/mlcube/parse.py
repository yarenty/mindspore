# Lint as: python3
"""Parses an mind-docker directory tree.

Will return objects which represent the metadata of the mind-docker.
"""
...
def parse_md_task(filename):
    ...
    for input_dict in task.inputs:
      input_obj, err = mlobject_from_dict('md_task_input', '1.0.0', input_dict)
      if err:
        return None, err
    ...
    for output_dict in task.outputs:
      output_obj, err = mlobject_from_dict('md_task_output', '1.0.0', output_dict)
      if err:
        return None, err
...
def parse_md_docker(filename):
    (docker, err) = MLObject.create_object_from_file(filename)
    return docker, err


def parse_md(root_dir):
  path = Path(root_dir, 'md.yaml').as_posix()
  if not os.path.exists(path):
    return None, 'root metadata does not exist: {}'.format(path)

  root, err = parse_md_root(path)
  if err:
    return None, err

  tasks = {}
  for task_file in root.tasks:
    task, err = parse_md_task(os.path.join(root_dir, task_file))
    if err:
      return None, err
    if task is None:
  ...
  docker, err = parse_md_docker(Path(root_dir, 'md_docker.yaml').as_posix())
  if err:
    return None, err

  return MindDockerMetadata(root, tasks, docker), None
