# Lint as: python3
"""Checks mind-docker metadata to ensure it is valid.
...
from minddocker.parse import parse_md

def check_root_dir(root_dir):
  metadata, err = parse_md(Path(root_dir).resolve().as_posix())
  if err:
    return None, err
...
def check_invoke_file(invoke_file):
  metadata, err = parse_md_invoke(Path(invoke_file).resolve().as_posix())
  if err:
    return None, err
...
def check_invoke_semantics_or_die(metadata, invoke):
  ...
  if not err:
    return
  print('FATAL invoke not valid for this mind-docker: {}'.format(err))
  sys.exit(1)
