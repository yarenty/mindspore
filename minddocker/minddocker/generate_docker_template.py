...
This takes an mind-docker directory which has metadata filled in
...
from minddocker import minddocker_check
...
ENTRYPOINT ["/usr/bin/python3", "/internal_docker_md_main.py"]
"""
...
def generate_function(task_name, io_names):
  ...
  command_line = '  return os.system("python3 internal_docker_md_task_{}.py '.format(task_name)
...
def generate_dispatch(task_names):
  ...
  for i, task_name in enumerate(task_names):
    if i == 0:
      ...
      code += '  elif task == "{}":\n'.format(task_name)
    code += '    if not md_{}(iomap):\n'.format(task_name)
    code += '      sys.exit(1)\n'
  code += '  else:\n'
  code += '    print("No known mind-docker task: {}".format(task))\n'
  code += '    sys.exit(1)\n'
  return code
...
def generate_readme_text(md_root, md, task_main_names):
  run_yamls = []
  for f in os.listdir(os.path.join(md_root, 'run')):
    if '.yaml' in f:
      run_yamls.append(os.path.join(os.path.join(md_root, 'run'), f))

  if len(run_yamls) == 0:
    run_text = 'You need to first create some run configs under {}'.format(os.path.join(md_root, 'run'))
  else:
    run_text = ''
    for run in run_yamls:
      run_text += 'python3 md_docker_run/docker_run.py --no-pull {}\n'.format(run)

  text = r"""Here is a starting point to create your MD's Docker Image.

Here are some notes to get started:
- Feel free to replace Dockerfile with an existing one you use!
- Make sure to use internal_docker_md_main.py as your main file (even in a
  different docker).


1. Each task in your mind-docker has a separate main file which was generated:
{task_mains}
...
""".format(task_mains=', '.join(task_main_names), docker_tag=md.docker.image, run_text=run_text)
  return text
...
def generate(mlcube_dir, mlcube):
  ...
  main_file_path = os.path.join(mlcube_dir, 'build', 'internal_docker_md_main.py')
  ...
  for task_name in task_main_texts:
    task_file_path = os.path.join(mlcube_dir, 'build', 'internal_docker_md_task_{}.py'.format(task_name))
...
def main():
  if len(sys.argv) != 2:
    ...
    sys.exit(-1)
  md_dir = sys.argv[1]
  md = minddocker_check.check_root_dir_or_die(md_dir)
  generate(md_dir, md)
